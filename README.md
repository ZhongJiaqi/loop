...
市面上的打卡 App 有两个直接的问题：功能越做越繁杂，核心功能还要订阅付费。更根本的是，它们全停在「行为」层——打卡链一断就整个弃用，因为行为没有挂在更深的东西上。行为科学里的身份改变回路是 thoughts → feelings → actions → identity：想法造感受，感受驱动行为，行为反过来塑造身份认同。所以自己做了一个**不是打卡工具**的东西：按 Dilts 逻辑层次把每日练习拆成三层——Affirmations（我是谁）、Mindsets（我怎么想）、Habits（我做什么）——一天一屏、三个子 tab 过完，思维、感受、行动的循环闭合才算完成。自己在用的真实产品，不是 demo。
